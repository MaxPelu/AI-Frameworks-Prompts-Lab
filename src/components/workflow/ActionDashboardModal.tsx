...
import {
  ...
  quickRefineStream,
  modifyContentLengthStream,
  isThinkingSupported,
} from "../../lib/geminiService.ts";
...
const ActionDashboardModal: React.FC<ActionDashboardModalProps> = ({
  ...
  const TONES = [
    {
      id: "neutral",
      name: "Neutral",
      prompt: "Mantén un tono neutral y equilibrado.",
    },
    {
      id: "formal",
      name: "Formal",
      prompt: "Usa un lenguaje formal, profesional y sofisticado.",
    },
    {
      id: "creative",
      name: "Creativo",
      prompt: "Sé creativo, usa metáforas y un lenguaje inspirador.",
    },
    {
      id: "casual",
      name: "Casual",
      prompt: "Usa un tono cercano, amigable y conversacional.",
    },
    {
      id: "technical",
      name: "Técnico",
      prompt: "Sé preciso, técnico y directo al grano.",
    },
  ];
  ...
  const getActionDetails = () => {
    switch (actionType) {
      case "expand":
        return {
          title: "Robustecimiento Estratégico",
          icon: <ArrowsPointingOutIcon className="w-6 h-6" />,
          color: "indigo",
        };
      case "optimize":
        return {
          title: "Optimización de Prompt",
      ...
      case "save":
        return {
          ...
          icon: <SaveDiskIcon className="w-6 h-6" />,
          color: "slate",
        };
      case "copy":
        return {
  ...
  const runProcess = async () => {
    ...
    if (isAIAction) {
      ...
      addLog(
        `Analizando texto base (${ideaText.length} caracteres)...`,
        "info",
      );
    }
    ...
    try {
      ...
      const modifiedSettings = {
        ...
        temperature,
        maxOutputTokens,
        thinkingBudget: isThinkingModel ? thinkingBudget : undefined,
        isThinkingMode: isThinkingModel,
      ...
      if (actionType === "expand") {
        addLog(
          "Generando expansión de contenido y enriquecimiento (Streaming)...",
          "info",
        );
        response = await expandIdeaStream(
          ideaText,
          files,
          framework,
          modifiedSettings,
        ...
        addLog(
          "Aplicando técnicas de Prompt Engineering (Streaming)...",
          "info",
        );
        response = await optimizePromptStream(
          ideaText,
          useCase,
          framework!,
          files,
        ...
        addLog(
          `Aplicando refinamiento rápido: ${actionType} (Streaming)...`,
          "info",
        );
        response = await quickRefineStream(
          ideaText,
          actionType as any,
          modifiedSettings,
          onChunk,
        ...
        response = await modifyContentLengthStream(
          ideaText,
          "simple",
          modifiedSettings,
          onChunk,
        ...
        response = {
          text: "Tu trabajo ha sido guardado exitosamente en el historial. Puedes acceder a él desde la biblioteca en cualquier momento.",
          usage: undefined,
        };
      ...
      addLog(
        `Error: ${error.message || "Fallo en la comunicación con la API"}`,
        "error",
      );
      setStatus("error");
    }
  ...
  const estimatedCost = useMemo(() => {
    ...
    const outputCost = (usage.candidatesTokens / 1000000) * 0.6;
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
      <div className="bg-slate-900 w-full max-w-6xl h-[90vh] rounded-3xl border border-white/10 shadow-2xl overflow-hidden flex flex-col">
        {/* Header */}
        <div
          className={`p-6 border-b border-white/10 flex justify-between items-center bg-gradient-to-r from-${details.color}-500/10 to-transparent`}
        >
          <div className="flex items-center gap-3">
            <div
              className={`p-2 bg-${details.color}-500/20 rounded-xl text-${details.color}-400`}
            >
              {details.icon}
            </div>
            <div>
              ...
              <div className="flex items-center gap-4 mt-1">
                <p className="text-sm text-gray-400 font-medium">
                  Dashboard de Procesamiento Avanzado
                </p>
                {usage && (
          ...
          <div className="w-80 border-r border-white/10 flex flex-col bg-black/20">
                ...
                <>
                  <div className="space-y-4">
                    <div className="flex flex-col gap-1.5">
                      ...
                      >
                        {MODELS.map((m) => (
                          <option key={m} value={m} className="bg-[#020617]">
                            {m}
                          </option>
                        ))}
                      </select>
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setMaxOutputTokens(parseInt(e.target.value))
                        }
                        className={`w-full accent-${details.color}-500`}
                      />
                    </div>
                    ...
                    {isThinkingModel && (
                      <div className="flex flex-col gap-1.5">
                        <div className="flex justify-between items-center">
                          <label className="text-[10px] text-teal-500 font-bold uppercase">
                            Thinking Tokens
                          </label>
                        ...
                        <input
                          type="range"
                          min="1024"
                          max={Math.min(65535, Math.max(1024, maxOutputTokens - 1024))}
                          step="1024"
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <div className="relative group">
                          <textarea
                            ...
                            className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 pr-8 text-xs text-white resize-none h-20 focus:outline-none focus:border-blue-500/50"
                        ...
                        {history.map((item) => (
                          <button
                            ...
                            onClick={() => {
                              ...
                              setCustomInstructions(
                                item.settings.customInstructions,
                              );
                            }}
                          ...
                          >
                            <div className="flex justify-between items-center">
                              ...
                              <span className="text-[9px] px-1 bg-white/10 rounded text-gray-300">
                                {item.settings.selectedModel.split("-")[1]}
                              </span>
                            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 custom-scrollbar bg-black/40 font-mono text-[10px]">
              <div className="space-y-2">
                ...
                {status === "running" && (
                  <div
                    className={`flex gap-2 text-${details.color}-400 animate-pulse`}
                  >
            ...
            {status === "running" || status === "completed" ? (
                ...
                <div className="flex justify-between items-center mb-4">
                      ...
                      <div className="flex items-center gap-3 text-[10px] text-gray-500 font-mono">
                        ...
                        <div className="w-20 h-1 bg-white/5 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${Math.min(100, (usage.totalTokens / 4096) * 100)}%`,
                            }}
                          />
                        </div>
                      </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      ...
                      {isCopied && (
                        <span className="absolute -top-8 left-1/2 -translate-x-1/2 bg-green-500 text-white text-[10px] px-2 py-1 rounded">
                          Copiado!
                        </span>
                      )}
                    </button>
                    ...
                    {onRunAutoFlow &&
                      ["expand", "optimize"].includes(actionType) && (
                        <button
                  ...
                  <div className="mb-6 p-4 bg-indigo-500/10 border border-indigo-500/20 rounded-2xl flex items-start gap-4 animate-fade-in">
                    ...
                    <div>
                      <h4 className="text-sm font-bold text-indigo-300">
                        Modo de Robustecimiento Activo
                      </h4>
                      <p className="text-xs text-gray-400 mt-1 leading-relaxed">
                        Este motor no intenta "responder" a tu idea, sino que la
                        desglosa, identifica capas ocultas y expande su
                        arquitectura conceptual para hacerla 10 veces más sólida
                    ...
                    <div className="flex-1 bg-black/40 border border-white/10 rounded-2xl p-6 overflow-y-auto custom-scrollbar text-gray-200 whitespace-pre-wrap text-base leading-relaxed selection:bg-purple-500/30">
                      {result ||
                        (status === "running"
                          ...
                          : "Acción completada exitosamente sin output de texto.")}
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
                ...
                <p className="text-gray-400 mb-6">
                  No se pudo completar el proceso. Revisa los logs para más
                  detalles técnicos.
                </p>
                <button
                  onClick={runProcess}
