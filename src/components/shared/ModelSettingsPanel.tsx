...
import { isThinkingSupported } from "../../lib/geminiService.ts";
...
interface ModelSettingsPanelProps {
  ...
  toneShift: number;
  setToneShift: (v: number) => void;
  draftMode: boolean;
  setDraftMode: (v: boolean) => void;
  promptAutoRefine: boolean;
  setPromptAutoRefine: (v: boolean) => void;
  verificationLoop: boolean;
  setVerificationLoop: (v: boolean) => void;
  outputVerbosity: number;
  setOutputVerbosity: (v: number) => void;
  expertiseLevel: number;
  setExpertiseLevel: (v: number) => void;
  forceMarkdown: boolean;
  setForceMarkdown: (v: boolean) => void;
  citationStyle: "none" | "apa" | "inline" | "url";
  setCitationStyle: (v: any) => void;
  lateralThinking: boolean;
  setLateralThinking: (v: boolean) => void;
  memoryRecall: boolean;
  setMemoryRecall: (v: boolean) => void;
  useGoogleMaps: boolean;
  setUseGoogleMaps: (v: boolean) => void;
  agentMathMode: boolean;
  setAgentMathMode: (v: boolean) => void;
  agentDebateMode: boolean;
  setAgentDebateMode: (v: boolean) => void;
  recursiveRefinement: number;
  setRecursiveRefinement: (v: number) => void;
  agentReflection: boolean;
  setAgentReflection: (v: boolean) => void;
  agentPlanningMode: boolean;
  setAgentPlanningMode: (v: boolean) => void;
  searchRecency: "any" | "day" | "week" | "month" | "year";
  setSearchRecency: (v: any) => void;
  groundingThreshold: number;
  setGroundingThreshold: (v: number) => void;
  agentSkills: AgentSkill[];
  setAgentSkills: (v: AgentSkill[]) => void;
}
...
const SectionHeader: React.FC<{ title: string; icon: React.ReactNode }> = ({
  ...
  <div className="flex items-center gap-2 mb-4 pt-2">
    ...
    <h4 className="font-bold text-gray-200 text-sm uppercase tracking-widest">
      {title}
    </h4>
...
const SliderControl: React.FC<{
  ...
  <div
    className={`space-y-2 group ${highlight ? "bg-teal-500/5 p-2 rounded-lg border border-teal-500/10" : ""}`}
  >
...
const Toggle: React.FC<{
  ...
  tooltip: string;
  warning?: string;
}> = ({ label, enabled, onToggle, disabled, tooltip, warning }) => (
  <div className="flex items-center justify-between group py-1">
    <div className="flex flex-col">
      <label
        className={`text-sm font-medium flex items-center gap-1.5 relative cursor-help ${disabled ? "text-gray-500" : "text-gray-300 hover:text-teal-300 transition-colors"}`}
      >
        {label}
      ...
      {warning && (
        <span className="text-orange-400 text-[9px] mt-0.5">{warning}</span>
      )}
    </div>
    <button
      type="button"
...
const ModelButton: React.FC<{
  ...
  >
    <div className="flex flex-col leading-tight text-center relative z-10">
      ...
      <span
        className={`text-[8px] sm:text-[9px] font-medium mt-0.5 ${current === id ? "text-white/90" : "text-gray-500 group-hover:text-gray-300"}`}
      >
        {sub}
      </span>
    </div>
    {current === id && (
      <div className="absolute top-0 right-0 p-1">
        <SparklesIcon className="w-2.5 h-2.5 text-white/50" />
      </div>
    )}
  </button>
);
...
const ModelSettingsPanel: React.FC<ModelSettingsPanelProps> = (props) => {
  ...
  const [activeTab, setActiveTab] = useState<
    "params" | "system" | "tools" | "skills" | "audio" | "advanced"
  >("params");
  ...
  // Skills State
  const [editingSkill, setEditingSkill] = useState<Partial<AgentSkill> | null>(
    null,
  );
  ...
  const [mirostatMode, setMirostatMode] = useState<"disabled" | "v1" | "v2">(
    "disabled",
  );
  ...
  >({ harassment: 2, hate: 2, sex: 2, danger: 2 });

  const contextUsagePercent = Math.min(
    ((settings.systemInstruction.length +
      settings.systemInstructionFiles.length * 1000) /
      128000) *
      100,
    100,
  );
  ...
  const handleLoadPreset = (preset: PresetConfig) => {
    ...
    settings.onIsStructuredOutputEnabledChange(
      preset.isStructuredOutputEnabled,
    );
    ...
    if (preset.groundingThreshold !== undefined)
      setGroundingThreshold(preset.groundingThreshold);
    if (preset.agentReflection !== undefined)
      setAgentReflection(preset.agentReflection);
    if (preset.agentPlanningMode !== undefined)
      setAgentPlanningMode(preset.agentPlanningMode);
    if (preset.searchRecency !== undefined)
      setSearchRecency(preset.searchRecency as any);

    // Load new features
    if (preset.useGoogleMaps !== undefined)
      setUseGoogleMaps(preset.useGoogleMaps);
    if (preset.agentMathMode !== undefined)
      setAgentMathMode(preset.agentMathMode);
    if (preset.agentDebateMode !== undefined)
      setAgentDebateMode(preset.agentDebateMode);
    if (preset.recursiveRefinement !== undefined)
      setRecursiveRefinement(preset.recursiveRefinement);
    if (preset.expertiseLevel !== undefined)
      setExpertiseLevel(preset.expertiseLevel);
    if (preset.outputVerbosity !== undefined)
      setOutputVerbosity(preset.outputVerbosity);
    if (preset.forceMarkdown !== undefined)
      setForceMarkdown(preset.forceMarkdown);
    if (preset.citationStyle !== undefined)
      setCitationStyle(preset.citationStyle as any);
    if (preset.lateralThinking !== undefined)
      setLateralThinking(preset.lateralThinking);
    if (preset.memoryRecall !== undefined) setMemoryRecall(preset.memoryRecall);

    if (preset.promptAutoRefine !== undefined)
      setPromptAutoRefine(preset.promptAutoRefine);
    if (preset.verificationLoop !== undefined)
      setVerificationLoop(preset.verificationLoop);
  ...
  const handleReset = () => {
    ...
    settings.onIsCodeExecutionEnabledChange(
      defaultSettings.isCodeExecutionEnabled,
    );
    settings.onIsFunctionCallingEnabledChange(
      defaultSettings.isFunctionCallingEnabled,
    );
  ...
  const handleAddStopSequence = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter" && currentStopSequence.trim()) {
      ...
      if (!settings.stopSequences.includes(currentStopSequence.trim())) {
        settings.onStopSequencesChange([
          ...settings.stopSequences,
          currentStopSequence.trim(),
        ]);
      }
  ...
  const handleRemoveStopSequence = (sequenceToRemove: string) => {
    settings.onStopSequencesChange(
      settings.stopSequences.filter((s) => s !== sequenceToRemove),
    );
  };
  ...
  const handleModelSelect = (model: GeminiModel) => {
    ...
    if (
      ...
      model !== "gemini-agent" &&
      model !== "google-antigravity-engine"
    ) {
  ...
  const isThinkingModel = isThinkingSupported(settings.selectedModel);
  const isNativeAudioSupported =
    settings.selectedModel === "gemini-2.5-flash-native-audio-preview-09-2025";
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
      <div
        ref={panelRef}
        className="bg-slate-900 w-full max-w-6xl h-[90vh] rounded-[3rem] border border-white/10 shadow-[0_30px_100px_-20px_rgba(0,0,0,0.9)] overflow-hidden flex flex-col ring-1 ring-white/5"
      >
        {/* --- HEADER & PRESETS --- */}
        <div className="p-8 border-b border-white/10 bg-white/5 backdrop-blur-3xl flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="font-black text-3xl text-gray-100 tracking-tight">
                Panel de Control{" "}
              ...
              <p className="text-gray-400 text-sm font-medium mt-1">
                Configuración Maestra de Motores de IA
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            <button
              onClick={handleReset}
              disabled={disabled}
              className="group flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-[11px] font-bold uppercase tracking-widest text-gray-400 hover:text-teal-400 hover:border-teal-500/30 transition-all"
            >
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="w-72 border-r border-white/10 bg-black/20 flex flex-col p-6 gap-2">
            <div className="mb-6">
              <h4 className="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] mb-4 px-2">
                Categorías
              </h4>
              ...
              <div className="bg-black/40 p-4 rounded-2xl border border-white/5">
                <div className="flex gap-2 mb-3">
                    ...
                    {presets.length > 0 && (
                      <div className="absolute bottom-full left-0 w-full mb-2 bg-slate-800 border border-white/10 rounded-xl shadow-2xl opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none group-hover:pointer-events-auto z-20 overflow-hidden max-h-48 overflow-y-auto">
                        {presets.map((p) => (
                          <div
                            key={p.id}
                            className="flex justify-between items-center px-3 py-2 hover:bg-white/5 border-b border-white/5 last:border-0"
                          >
                            <button
                              onClick={() => handleLoadPreset(p)}
                              className="text-left text-[10px] font-medium text-gray-200 hover:text-teal-400 flex-1 truncate"
                            >
                              {p.name}
                            </button>
                            <button
                  ...
                  <button
                    ...
                    className="bg-teal-500/10 text-teal-400 border border-teal-500/30 p-2 rounded-xl hover:bg-teal-500/20 transition-all"
                  >
                ...
                {isSavingPreset && (
                  <div className="space-y-2 animate-fade-in">
                    <input
                      ...
                      placeholder="Nombre..."
                      className="bg-black/40 border border-white/10 w-full text-[10px] p-2 rounded-lg text-white focus:outline-none focus:border-teal-500/50"
                    />
                    <button
                      onClick={handleSavePreset}
                      disabled={!newPresetName}
                      className="w-full bg-teal-600/20 border border-teal-500/30 text-teal-100 py-2 rounded-lg text-[10px] font-black hover:bg-teal-600/30 disabled:opacity-50"
                    >
                      GUARDAR ACTUAL
                    </button>
                  </div>
          ...
          <div className="flex-1 overflow-y-auto p-10 custom-scrollbar bg-slate-900/50 relative">
            ...
            <div className="absolute top-6 right-10 flex items-center gap-3 bg-teal-500/10 border border-teal-500/20 px-4 py-2 rounded-2xl animate-fade-in">
              ...
              <span className="text-[10px] font-black text-teal-400 uppercase tracking-widest">
                Motor Activo: {settings.selectedModel}
              </span>
            </div>
              ...
              <div className="space-y-6 animate-fade-in">
                <section>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mb-4">
                    ...
                    />
                    <ModelButton
                      id="gemini-3.1-flash-lite-preview-super-high"
                      label="3.1 Lite (Super)"
                      sub="Max Thinking"
                      highlight
                      current={settings.selectedModel}
                      onClick={handleModelSelect}
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mb-4">
                    ...
                    />
                    <ModelButton
                      id="gemini-3-pro-preview-medium"
                      label="3 Pro (Med)"
                      sub="Med Thinking"
                      highlight
                      current={settings.selectedModel}
                    ...
                    />
                    <ModelButton
                      id="gemini-3-pro-preview-super-high"
                      label="3 Pro (Super)"
                      sub="Max Thinking"
                      highlight
                      current={settings.selectedModel}
                    ...
                    />
                    <ModelButton
                      id="gemini-3-flash-preview-super-high"
                      label="3 Flash (Super)"
                      sub="Max Thinking"
                      highlight
                      current={settings.selectedModel}
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-4">
                    <ModelButton
                      ...
                      label="2.5 Pro Latest"
                      sub="Balanced"
                      colorClass="bg-blue-300/80 border-blue-200"
                      current={settings.selectedModel}
                      onClick={handleModelSelect}
                      disabled={disabled}
                    />
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-4">
                    <ModelButton
                      ...
                      label="2.0 Pro Exp"
                      sub="Experimental"
                      colorClass="bg-indigo-600/80 border-indigo-400"
                      current={settings.selectedModel}
                      onClick={handleModelSelect}
                      disabled={disabled}
                    />
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-4">
                    <ModelButton
                      ...
                      label="1.5 Pro"
                      sub="Legacy"
                      colorClass="bg-gray-600/80 border-gray-400"
                      current={settings.selectedModel}
                      onClick={handleModelSelect}
                      disabled={disabled}
                    />
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-4">
                    <ModelButton
                      ...
                      label="Audio 09-2025"
                      sub="Native Audio"
                      colorClass="bg-pink-600/80 border-pink-400"
                      current={settings.selectedModel}
                      onClick={handleModelSelect}
                      disabled={disabled}
                    />
                    ...
                    <ModelButton
                      ...
                      label="2.5 Image"
                      sub="Image Gen"
                      colorClass="bg-purple-600/80 border-purple-400"
                      current={settings.selectedModel}
                      onClick={handleModelSelect}
                      disabled={disabled}
                    />
                    ...
                    <ModelButton
                      ...
                      label="Veo Fast"
                      sub="Video Gen"
                      colorClass="bg-orange-600/80 border-orange-400"
                      current={settings.selectedModel}
                      onClick={handleModelSelect}
                      disabled={disabled}
                    />
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-2">
                    ...
                    <ModelButton
                      ...
                      label="Gemma 3"
                      sub="27B Instruct"
                      colorClass="bg-emerald-600/80 border-emerald-400"
                      current={settings.selectedModel}
                      onClick={handleModelSelect}
                      disabled={disabled}
                    />
                ...
                <section className="bg-white/5 rounded-2xl p-5 border border-white/5 space-y-5">
                  ...
                  <Toggle
                    label="Draft Mode (Low Cost)"
                    enabled={draftMode}
                    onToggle={setDraftMode}
                    disabled={disabled}
                    tooltip="Modo rápido y económico para iteraciones iniciales."
                  />
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <SliderControl
                      ...
                      max={2}
                      step={0.1}
                      onChange={settings.onTemperatureChange}
                      disabled={disabled}
                      tooltip="Aleatoriedad. 0=Determinista, 1+=Creativo."
                    />
                    <SliderControl
                      label="Top P (Nucleus)"
                      value={settings.topP}
                      ...
                      max={1}
                      step={0.01}
                      onChange={settings.onTopPChange}
                      disabled={disabled}
                      tooltip="Corte de masa de probabilidad acumulada."
                    />
                    <SliderControl
                      label="Top K"
                      value={settings.topK}
                      ...
                      max={100}
                      step={1}
                      onChange={settings.onTopKChange}
                      disabled={disabled}
                      tooltip="Corte de top K tokens."
                    />
                    <SliderControl
                      ...
                      max={99999}
                      step={1}
                      onChange={(v) => settings.onSeedChange(v)}
                      disabled={disabled}
                ...
                <section className="bg-white/5 rounded-2xl p-5 border border-white/5 space-y-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <SliderControl
                      ...
                      max={2}
                      step={0.1}
                      onChange={settings.onFrequencyPenaltyChange}
                      disabled={disabled}
                      tooltip="Penaliza tokens frecuentes (reduce repetición)."
                      highlight
                    />
                    <SliderControl
                      ...
                      max={2}
                      step={0.1}
                      onChange={settings.onPresencePenaltyChange}
                      disabled={disabled}
                      tooltip="Penaliza tokens si ya aparecieron (aumenta variedad)."
                      highlight
                    />
                    <SliderControl
                      ...
                      disabled={disabled}
                      tooltip="Penalización multiplicativa."
                    />
                    <SliderControl
                      label="Min P (SOTA)"
                      value={minP}
                      ...
                      onChange={setMinP}
                      disabled={disabled}
                      tooltip="Corte relativo a la probabilidad del top token."
                    />
                    <SliderControl
                      label="Top A"
                      value={topA}
                      ...
                      onChange={setTopA}
                      disabled={disabled}
                      tooltip="Considera tokens con prob^2 > Top A."
                    />
                  </div>
                </section>
              </div>
              ...
              <div className="space-y-6 animate-fade-in">
                <section className="space-y-3">
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      settings.onSystemInstructionChange(e.target.value)
                    }
                    placeholder="Define el rol y comportamiento de la IA..."
                    rows={6}
                    className="glass-input w-full text-sm rounded-2xl p-4 text-gray-200 resize-none shadow-inner border border-white/5 focus:border-teal-500/50 transition-all font-sans"
                    disabled={disabled}
                  />
                  <FileUploader
                    files={settings.systemInstructionFiles}
                    onSystemInstructionFilesChange={
                      settings.onSystemInstructionFilesChange
                    }
                  />
                </section>
                ...
                <section className="bg-white/5 rounded-2xl p-5 border border-white/5 space-y-4">
                  ...
                  <SliderControl
                    ...
                    disabled={disabled}
                    tooltip="Simula una ventana de contexto más pequeña para pruebas."
                  />

                  <div className="space-y-2">
                    <label className="text-xs font-semibold text-gray-400">
                      Stop Sequences (Enter to add)
                    </label>
                    <div className="flex gap-2 flex-wrap bg-black/20 p-2 rounded-lg border border-white/5 min-h-[40px]">
                      {settings.stopSequences.map((seq, idx) => (
                        <span
                          key={idx}
                          className="bg-red-500/20 text-red-300 px-2 py-1 rounded text-xs flex items-center gap-1 border border-red-500/30"
                        >
                          {seq}{" "}
                          <button
                ...
                <section className="bg-red-900/10 rounded-2xl p-5 border border-red-500/20 space-y-4">
                  <SectionHeader
                    ...
                    icon={<ShieldCheckIcon className="w-4 h-4 text-red-400" />}
                  />
                  <div className="grid grid-cols-2 gap-4">
                    <SliderControl
                      label="Harassment"
                      value={safetyThresholds.harassment}
                      min={0}
                      max={3}
                      step={1}
                      onChange={(v) =>
                      ...
                      disabled={disabled}
                      tooltip="0=None, 3=High"
                    />
                    <SliderControl
                      label="Hate Speech"
                      value={safetyThresholds.hate}
                      min={0}
                      max={3}
                      step={1}
                      onChange={(v) =>
                        setSafetyThresholds({ ...safetyThresholds, hate: v })
                      }
                      disabled={disabled}
                      tooltip="0=None, 3=High"
                    />
                    <SliderControl
                      ...
                      value={safetyThresholds.sex}
                      min={0}
                      max={3}
                      step={1}
                      onChange={(v) =>
                        setSafetyThresholds({ ...safetyThresholds, sex: v })
                      }
                      disabled={disabled}
                      tooltip="0=None, 3=High"
                    />
                    <SliderControl
                      label="Dangerous"
                      value={safetyThresholds.danger}
                      min={0}
                      max={3}
                      step={1}
                      onChange={(v) =>
                        setSafetyThresholds({ ...safetyThresholds, danger: v })
                      }
                      disabled={disabled}
                      tooltip="0=None, 3=High"
                    />
                  ...
                  <button
                    ...
                    className="w-full py-2 bg-red-500/10 text-red-400 border border-red-500/30 rounded-lg text-xs font-bold uppercase hover:bg-red-500/20 transition-all"
                  >
            ...
            {activeTab === "tools" && (
              <div className="space-y-6 animate-fade-in">
                <section className="bg-white/5 rounded-2xl p-5 border border-white/5 space-y-5">
                  ...
                  <div className="p-3 bg-teal-900/20 rounded-xl border border-teal-500/10 mb-2">
                    <Toggle
                      ...
                      enabled={settings.useGoogleSearch}
                      onToggle={settings.onUseGoogleSearchChange}
                      disabled={disabled}
                      tooltip="Búsqueda web en tiempo real con Gemini Grounding."
                    />
                    {settings.useGoogleSearch && (
                      <div className="mt-3 pl-2 border-l-2 border-teal-500/30 space-y-3 animate-fade-in">
                        <SliderControl
                          ...
                          max={1.0}
                          step={0.1}
                          onChange={setGroundingThreshold}
                          disabled={disabled}
                          tooltip="Fidelidad a la fuente: 1.0 es estricto, 0.0 permite alucinación creativa."
                        />
                        <div className="flex items-center justify-between">
                          ...
                          <select
                            value={searchRecency}
                            onChange={(e) =>
                              setSearchRecency(e.target.value as any)
                            }
                            className="bg-black/30 border border-white/10 rounded-md text-[10px] text-teal-400 px-2 py-1 outline-none"
                          >
                  ...
                  <Toggle
                    label="Tool: Google Maps"
                    enabled={useGoogleMaps}
                    onToggle={setUseGoogleMaps}
                    disabled={disabled}
                    tooltip="Acceso a datos geo-espaciales reales."
                  />
                  <Toggle
                    label="Code Sandbox (Python)"
                    enabled={settings.isCodeExecutionEnabled}
                    onToggle={settings.onIsCodeExecutionEnabledChange}
                    disabled={disabled}
                    tooltip="Ejecución de código en sandbox."
                  />
                  <Toggle
                    label="Function Calling (Auto)"
                    enabled={settings.isFunctionCallingEnabled}
                    onToggle={settings.onIsFunctionCallingEnabledChange}
                    disabled={disabled}
                    tooltip="Permite al modelo detectar y llamar funciones."
                  />
                </section>
                ...
                <section className="bg-purple-900/10 rounded-2xl p-5 border border-purple-500/20 space-y-5 relative overflow-hidden">
                  ...
                  <SectionHeader
                    title="Agentic Cognitive Layers"
                    icon={<UsersIcon className="w-4 h-4 text-purple-400" />}
                  />

                  <Toggle
                    label="Prompt Auto-Refine"
                    enabled={promptAutoRefine}
                    onToggle={setPromptAutoRefine}
                    disabled={disabled}
                    tooltip="Mejora automáticamente el prompt antes de enviarlo al modelo."
                  />
                  <Toggle
                    label="Auto-Reflection Loop"
                    enabled={agentReflection}
                    onToggle={setAgentReflection}
                    disabled={disabled}
                    tooltip="El agente critica su propia respuesta antes de enviarla."
                  />
                  <Toggle
                    label="Step-by-Step Planning"
                    enabled={agentPlanningMode}
                    onToggle={setAgentPlanningMode}
                    disabled={disabled}
                    tooltip="Fuerza al modelo a generar un <plan> antes de ejecutar."
                  />
                  <Toggle
                    label="Debate Protocol"
                    enabled={agentDebateMode}
                    onToggle={setAgentDebateMode}
                    disabled={disabled}
                    tooltip="Genera Tesis y Antítesis antes de la Síntesis final."
                  />
                  <Toggle
                    label="Math/Logic Mode"
                    enabled={agentMathMode}
                    onToggle={setAgentMathMode}
                    disabled={disabled}
                    tooltip="Prioriza el uso de herramientas de cálculo y lógica formal."
                  />
                  <Toggle
                    label="Memory Recall (RAG Sim)"
                    enabled={memoryRecall}
                    onToggle={setMemoryRecall}
                    disabled={disabled}
                    tooltip="Simula la inyección de contexto previo relevante."
                  />
                  <Toggle
                    label="Verification Loop"
                    enabled={verificationLoop}
                    onToggle={setVerificationLoop}
                    disabled={disabled}
                    tooltip="Verifica hechos generados contra la base de conocimiento."
                  />
                  <Toggle
                    label="Lateral Thinking"
                    enabled={lateralThinking}
                    onToggle={setLateralThinking}
                    disabled={disabled}
                    tooltip="Fuerza analogías inusuales y soluciones creativas."
                  />

                  <SliderControl
                    label="Recursive Refinement Loop"
                    value={recursiveRefinement}
                    min={0}
                    max={3}
                    step={1}
                    onChange={setRecursiveRefinement}
                    disabled={disabled}
                    tooltip="Número de veces que el modelo iterará sobre su propia respuesta (0 = Off)."
                  />
                </section>
                ...
                <section className="bg-white/5 rounded-2xl p-5 border border-white/5 space-y-5">
                  ...
                  {settings.isStructuredOutputEnabled && (
                    <div className="space-y-2 animate-fade-in">
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          settings.onResponseSchemaChange(e.target.value)
                        }
                  ...
                  <Toggle
                    label="Force Markdown"
                    enabled={forceMarkdown}
                    onToggle={setForceMarkdown}
                    disabled={disabled}
                    tooltip="Asegura formato rico (listas, headers, código)."
                  />
                  <SliderControl
                    ...
                    disabled={disabled}
                    tooltip="1=Novato, 5=Súper-Experto (God Mode)."
                  />
                  <SliderControl
                    label="Verbosity"
                    ...
                    max={1.0}
                    step={0.1}
                    onChange={setOutputVerbosity}
                    disabled={disabled}
                    tooltip="0=Conciso (Haiku), 1=Extenso (Novela)."
                  />
                  <SliderControl
                    label="Tone Shift"
                    value={toneShift}
                    ...
                    max={5}
                    step={1}
                    onChange={setToneShift}
                    disabled={disabled}
                    tooltip="-5 (Serio/Triste) a +5 (Alegre/Divertido)."
                  />
                ...
                <section className="bg-indigo-900/20 rounded-2xl p-5 border border-indigo-500/20 space-y-5">
                  ...
                  >
                    <SliderControl
                      ...
                      min={0}
                      max={Math.min(65535, Math.max(1024, settings.maxOutputTokens - 1024))}
                      step={1024}
                      onChange={(v) => {
                        settings.onThinkingBudgetChange(v);
                        settings.onIsThinkingModeChange(v > 0);
                      }}
                      disabled={disabled || !isThinkingModel}
                      tooltip="Tokens reservados para pensamiento oculto."
                      highlight
                    />
                    <div className="flex gap-2">
                      {["shallow", "medium", "deep"].map((d) => (
                        <button
                          key={d}
                          className="flex-1 py-2 bg-indigo-500/10 border border-indigo-500/30 rounded-lg text-xs text-indigo-300 uppercase font-bold hover:bg-indigo-500/20"
                        >
                          {d}
                        </button>
                      ))}
                ...
                <section className="bg-white/5 rounded-2xl p-5 border border-white/5 space-y-5">
                  ...
                  {(!agentSkills || agentSkills.length === 0) && (
                    <div className="text-center py-8 text-gray-500 text-xs">
                      No custom skills added yet. Add skills like "Gemini
                      Antigravity", "Codex", or custom tool definitions.
                    </div>
                  )}
                  ...
                  <div className="space-y-3">
                    {agentSkills?.map((skill) => (
                      <div
                        key={skill.id}
                        className="bg-black/20 p-3 rounded-xl border border-white/5 hover:border-teal-500/30 transition-all group"
                      >
                        <div className="flex justify-between items-start mb-2">
                          <div className="flex items-center gap-2">
                            <Toggle
                              ...
                              onToggle={(v) => {
                                const updated = agentSkills.map((s) =>
                                  s.id === skill.id ? { ...s, enabled: v } : s,
                                );
                            ...
                            <div>
                              ...
                              <span className="text-[10px] text-gray-500 uppercase tracking-wider bg-white/5 px-1.5 py-0.5 rounded">
                                {skill.format}
                              </span>
                            </div>
                          </div>
                          <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <button
                              onClick={() => {
                                setAgentSkills(
                                  agentSkills.filter((s) => s.id !== skill.id),
                                );
                              }}
              ...
              <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
                <div className="bg-slate-900 w-full max-w-lg rounded-2xl border border-white/10 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                  ...
                  <div className="p-4 border-t border-white/10 bg-white/5 flex justify-end gap-2">
                    <button
                      onClick={() => setIsSkillModalOpen(false)}
                      className="px-4 py-2 rounded-lg text-xs font-bold text-gray-400 hover:bg-white/5"
                    >
                      Cancel
                    </button>
                    <button
                      onClick={() => {
                        if (editingSkill.id && editingSkill.name) {
                          ...
                          const existingIndex = agentSkills?.findIndex(
                            (s) => s.id === newSkill.id,
                          );
            ...
            {activeTab === "audio" && (
              <div className="space-y-6 animate-fade-in">
                <section
                  className={`bg-white/5 rounded-2xl p-5 border border-white/5 space-y-5 ${!isNativeAudioSupported ? "opacity-50 pointer-events-none" : ""}`}
                >
                  <SectionHeader
                    title="Native Voice Config"
                    icon={<ArrowUpTrayIcon className="w-4 h-4" />}
                  />
                  {!isNativeAudioSupported && (
                    <p className="text-xs text-red-400 font-bold mb-2">
                      Selecciona un modelo 'Native Audio' para habilitar.
                    </p>
                  )}

                  <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
                    {["Puck", "Charon", "Kore", "Fenrir", "Zephyr"].map(
                      (voice) => (
                        <button
                          key={voice}
            ...
            {activeTab === "advanced" && (
              <div className="space-y-6 animate-fade-in">
                <section className="bg-yellow-900/10 rounded-2xl p-5 border border-yellow-500/20 space-y-5">
                  <SectionHeader
                    title="Observability (Logprobs)"
                    icon={<BeakerIcon className="w-4 h-4 text-yellow-400" />}
                  />
                  <Toggle
                    label="Enable Logprobs"
                    enabled={logprobsEnabled}
                    onToggle={setLogprobsEnabled}
                    disabled={disabled}
                    tooltip="Ver probabilidades de tokens."
                  />
                  {logprobsEnabled && (
                    <SliderControl
                      label="Top Logprobs Count"
                      value={topLogprobs}
                      min={1}
                      max={5}
                      step={1}
                      ...
                      disabled={disabled}
                      tooltip="Cantidad de alternativas a mostrar."
                    />
                  )}
                </section>
                ...
                <section className="bg-white/5 rounded-2xl p-5 border border-white/5 space-y-5">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <Toggle
                      label="Mirostat V2"
                      enabled={mirostatMode === "v2"}
                      onToggle={(v) => setMirostatMode(v ? "v2" : "disabled")}
                      disabled={disabled}
                      tooltip="Control dinámico de perplejidad."
                    />
                    <Toggle
                      ...
                      onToggle={() => {}}
                      disabled={true}
                      tooltip="Experimental: TFS (Coming Soon)."
                    />
                    <Toggle
                      label="Typical P"
                      enabled={false}
                      onToggle={() => {}}
                      disabled={true}
                      tooltip="Experimental: Typical Sampling (Coming Soon)."
                    />
                    <Toggle
                      label="Echo Prompt"
                      enabled={false}
                      onToggle={() => {}}
                      disabled={true}
                      tooltip="Repite el prompt en la salida."
                    />
                  </div>
                </section>
                ...
                <div className="p-4 bg-black/40 rounded-xl border border-white/10 text-xs text-gray-500 font-mono">
                  ...
                  <br />
                  {JSON.stringify(
                    { ...settings, minP, topA, repetitionPenalty, voiceName },
                    null,
                    2,
                  ).substring(0, 300)}
                  ...
