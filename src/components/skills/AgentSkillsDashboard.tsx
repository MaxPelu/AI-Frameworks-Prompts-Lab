...
const AgentSkillsDashboard: React.FC<AgentSkillsDashboardProps> = ({
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          temperature: 0.7,
          maxOutputTokens:
            isThinkingModel && maxOutputTokens <= thinkingBudget
              ? thinkingBudget + 2048
              : maxOutputTokens,
          ...(isThinkingModel
            ? {
                thinkingConfig: {
                  thinkingBudget: Math.min(65535, thinkingBudget),
                },
              }
            : {}),
        },
      ...
      alert(
        "Failed to generate skill. Please check your API key and try again.",
      );
    } finally {
  ...
  const content = (
    <div
      className={`${inlineMode ? "h-full flex flex-col" : "fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in"}`}
    >
      <div
        className={`${inlineMode ? "flex-1 flex flex-col overflow-hidden" : "bg-slate-900 w-full max-w-5xl h-[85vh] rounded-3xl border border-white/10 shadow-2xl overflow-hidden flex flex-col"}`}
      >
        {/* Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-teal-200/60 font-medium">
                Manage capabilities for Gemini, Claude, Codex, Antigravity, etc.
              </p>
            </div>
          </div>
          <button
          ...
          <div className="w-1/3 border-r border-white/10 flex flex-col bg-black/20">
            <div className="p-4 border-b border-white/10 space-y-3">
              ...
              <div className="flex gap-2">
                ...
                >
                  <DocumentTextIcon className="w-4 h-4" /> Import Files (.md,
                  .json...)
                </button>
              </div>
                ...
                filteredSkills.map((skill) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-1">
                      ...
                      <span className="text-[9px] uppercase tracking-wider bg-black/40 px-1.5 py-0.5 rounded text-gray-400">
                        {skill.format}
                      </span>
                    </div>
            ...
            {editingSkill ? (
                ...
                <div className="flex justify-between items-center mb-6">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      onClick={() => {
                        setAgentSkills(
                          agentSkills.filter((s) => s.id !== editingSkill.id),
                        );
                        setEditingSkill(null);
                      }}
                ...
                <div className="space-y-5">
                  ...
                  <div className="flex-1 flex flex-col min-h-[300px]">
                    <label className="text-xs font-bold text-gray-400 block mb-1.5 uppercase tracking-wider flex justify-between items-center">
                      ...
                      <span className="text-[10px] text-teal-400/70 normal-case">
                        Paste raw markdown, code, or JSON here
                      </span>
                    </label>
                    <textarea
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
                ...
                <h3 className="text-2xl font-bold text-white mb-2">
                  Generate a Skill with Gemini
                </h3>
                <p className="text-gray-400 max-w-md mb-8">
                  Describe the capability you want to add, and Gemini will
                  generate a structured skill definition for you.
                </p>

                <div className="w-full max-w-xl bg-black/40 p-2 rounded-2xl border border-white/10 flex flex-col gap-2 shadow-inner">
                  <div className="flex flex-col gap-3 px-2 pt-2 pb-2 border-b border-white/5">
                    <div className="flex items-center gap-2">
                      <label className="text-xs font-bold text-gray-400 uppercase tracking-wider w-24">
                        Model:
                      </label>
                      <select
                        value={selectedModel}
                        ...
                        className="flex-1 bg-slate-800 text-white text-xs rounded-lg px-2 py-1.5 border border-white/10 focus:ring-1 focus:ring-teal-500/50 outline-none"
                    ...
                    {isThinkingModel && (
                      <div className="flex items-center gap-2">
                        <label className="text-xs font-bold text-teal-500 uppercase tracking-wider w-24">
                          Thinking:
                        </label>
                        <div className="flex-1 flex items-center gap-2">
                          <input
                  ...
                  <div className="flex items-center relative">
                    <input
                      ...
                      onKeyDown={(e) =>
                        e.key === "Enter" && handleGenerateSkill()
                      }
                      placeholder="e.g. Create a skill for analyzing React performance..."
                      className="flex-1 bg-transparent border-none outline-none text-white px-4 py-3 pr-10 placeholder-gray-500"
                      disabled={isGenerating}
                    />
                    {generationPrompt && (
                        <button 
                            onClick={() => setGenerationPrompt('')} 
                            title="Limpiar prompt" 
                            className="absolute right-[140px] p-1.5 rounded-md text-gray-400 hover:text-red-400 transition-colors"
                        >
