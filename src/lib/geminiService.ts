...
export const isThinkingSupported = (modelId: string): boolean => {
  return (
...
if (!ai) {
  console.warn(
    "GEMINI_API_KEY environment variable not set. Gemini features will not work.",
  );
}
...
const getErrorMessage = (feature: string) =>
  `Error: La API Key de Gemini no está configurada. La función "${feature}" no está disponible.`;
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      if ((isRateLimit || isServerError) && i < retries - 1) {
        console.warn(
          `API Error (${error?.status || "Unknown"}). Retrying in ${delay}ms... (Attempt ${i + 1} of ${retries})`,
        );
...
const callGeminiStreamWithRetry = async (
  params: any,
  retries = 3,
  initialDelay = 1000,
): Promise<any> => {
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      if ((isRateLimit || isServerError) && i < retries - 1) {
        console.warn(
          `API Error (Stream). Retrying in ${delay}ms... (Attempt ${i + 1} of ${retries})`,
        );
        await new Promise((resolve) => setTimeout(resolve, delay));
        delay *= 2; // Exponential backoff
      } else {
...
const constructSotaSystemInstruction = (settings: ModelSettings): string => {
  ...
  if (settings.expertiseLevel) {
    ...
    if (lvl === 5)
      role =
        "SOTA Research Entity (God Mode). Use maximal technical precision, academic rigor, and advanced abstractions.";
  ...
  if (settings.toneShift !== undefined && settings.toneShift !== 0) {
    ...
    let tone = "Neutral";
    if (t <= -4)
      tone = "Extremely Serious, Clinical, Stoic, and Dry. No emotion.";
    else if (t <= -2) tone = "Formal, Professional, and Corporate.";
    else if (t >= 4)
      tone = "Highly Energetic, Enthusiastic, Emoji-rich, and Inspiring.";
  ...
  if (settings.outputVerbosity !== undefined) {
    ...
    if (v <= 0.2)
      lengthInstr =
        "EXTREME BREVITY. Use Haiku-like efficiency. High information density, zero fluff. Bullet points preferred.";
    else if (v <= 0.4) lengthInstr = "Concise. Get straight to the point.";
    else if (v >= 0.8)
      lengthInstr =
        "Comprehensive and Exhaustive. Leave no detail out. Expand on every concept deeply.";
  ...
  if (settings.lateralThinking)
    cognitive.push(
      "- Lateral Thinking: Make unexpected connections and analogies.",
    );
  if (settings.agentDebateMode)
    cognitive.push(
      "- Debate Protocol: Internally evaluate thesis vs antithesis before outputting synthesis.",
    );
  if (settings.agentReflection)
    cognitive.push(
      "- Reflection: Critique your own output for errors before finalizing.",
    );
  ...
  if (settings.agentSkills && settings.agentSkills.length > 0) {
    ...
    if (enabledSkills.length > 0) {
      const skillsText = enabledSkills
        .map(
          (s) => `
            --- SKILL: ${s.name} ---
            [Format: ${s.format}]
...
const buildConfig = (
  ...
  overrideJsonMode: boolean = false,
) => {
  const config: any = {};
  ...
  if (typeof settings.topK === "number")
    config.topK = Math.floor(settings.topK);
  ...
  if (settings.stopSequences && settings.stopSequences.length > 0)
    config.stopSequences = settings.stopSequences;
  if (settings.seed !== null) config.seed = Math.floor(settings.seed);

  // Thinking Logic
  if (
    settings.isThinkingMode &&
    settings.thinkingBudget > 0 &&
    isThinkingSupported(settings.selectedModel)
  ) {
    config.thinkingConfig = {
      thinkingBudget: Math.min(65535, Math.floor(settings.thinkingBudget)),
    };
    // Validar que maxOutputTokens sea mayor que thinkingBudget
    if (
      config.maxOutputTokens &&
      config.maxOutputTokens <= config.thinkingConfig.thinkingBudget
    ) {
  ...
  if (
    ...
    if (settings.responseSchema) {
      try {
        config.responseSchema = JSON.parse(settings.responseSchema);
      } catch (e) {}
    }
  } else {
  ...
  if (settings.responseModalities && settings.responseModalities.length > 0) {
    ...
    if (
      settings.responseModalities.includes("AUDIO") &&
...
const extractUsage = (
  ...
  if (response && response.usageMetadata) {
    ...
    if ((usage as any).candidatesTokensDetails?.thinkingTokenCount) {
      thinkingTokens = (usage as any).candidatesTokensDetails
        .thinkingTokenCount;
    ...
    const snapshot: ConfigSnapshot | undefined = settings
      ? {
          ...
          systemInstructionSnippet: settings.systemInstruction
            ? settings.systemInstruction.substring(0, 100)
            : undefined,
...
Return ONLY the upgraded, ready-to-use prompt text.`;
    ...
    case "fix":
      instruction =
        "Goal: Strictly correct grammar, spelling, and punctuation. Do not change style.";
      break;
    case "translate":
      instruction =
        "Goal: Translate accurately between English and Spanish, maintaining tone.";
      break;
  }
...
export const expandIdea = async (
  ...
  const contextFromFiles = textBasedFiles
    .map(
      (f) => `[CONTEXT FILE: ${f.name}]\n${f.textContent?.substring(0, 8000)}`,
    )
    .join("\n\n");
  ...
  try {
    const response = await callGeminiWithRetry({
      model: modelToUse,
      contents: {
        parts: [textPart, ...files.map(fileToPart).filter((p) => p.inlineData)],
      },
      config: config,
    });
...
export const evaluatePromptQuality = async (
  promptText: string,
  settings: ModelSettings,
  ...
  const systemInstruction =
    "Analiza la calidad técnica del siguiente prompt para un LLM. Proporciona puntuaciones del 0 al 100 y feedback conciso.";
  ...
  try {
    ...
    parsed.usage = extractUsage(
      result,
      fastSettings.selectedModel,
      "analysis",
      undefined,
      promptText,
    );
...
export const suggestUseCase = async (
  idea: string,
  settings: ModelSettings,
): Promise<{ text: string; usage?: TokenUsage }> => {
  ...
  const response = await callGeminiWithRetry({
    model: resolveModel(fastSettings.selectedModel),
    contents: prompt,
    config: buildConfig(fastSettings, true),
  });
  return {
    text: response.text?.trim() || USE_CASES[0],
    usage: extractUsage(
      response,
      fastSettings.selectedModel,
      "analysis",
...
export const suggestFramework = async (
  idea: string,
  settings: ModelSettings,
): Promise<{ text: string; usage?: TokenUsage }> => {
  if (!ai) return { text: FRAMEWORKS?.[0]?.acronym || "RACE" };
  const prompt = `Basado en esta idea: "${idea}", sugiere el acrónimo del framework de prompting más adecuado de esta lista: ${ALL_FRAMEWORKS_LIST.map(
    (f) => f?.acronym,
  )
    .filter(Boolean)
    .join(", ")}. Responde solo con el acrónimo.`;
  ...
  return {
    text: response.text?.trim() || FRAMEWORKS?.[0]?.acronym || "RACE",
    usage: extractUsage(
      response,
      fastSettings.selectedModel,
      "analysis",
...
export const generateContent = async (
  p: string,
  s: any,
): Promise<{ text: string; thought?: string; usage?: TokenUsage }> => {
  if (!ai) return { text: "Error: API no configurada." };
  const response = await callGeminiWithRetry({
    model: resolveModel(s.selectedModel),
    contents: p,
...
export const LENGTH_MODIFIERS_CONFIG = {
  shorter: {
    label: "Más Corto",
    description: "Condensar información",
    prompt:
      "Reescribe el texto para que sea más breve y directo, eliminando redundancias.",
  },
  longer: {
    label: "Más Largo",
    description: "Añadir detalle",
    prompt:
      "Expande el texto añadiendo explicaciones detalladas, ejemplos y contexto adicional.",
  },
  bullets: {
    label: "Viñetas",
    description: "Lista estructurada",
    prompt: "Convierte el texto en una lista de viñetas clara y organizada.",
  },
  simple: {
    label: "Simplificar",
    description: "Lenguaje sencillo",
    prompt:
      "Reescribe el texto usando un lenguaje simple y fácil de entender (Nivel 5to grado).",
  },
  formal: {
    label: "Formal",
    description: "Tono profesional",
    prompt: "Reescribe el texto con un tono formal, profesional y corporativo.",
  },
  casual: {
    label: "Casual",
    description: "Tono relajado",
    prompt: "Reescribe el texto con un tono casual, amigable y conversacional.",
  },
} as const;

export type LengthModifier = keyof typeof LENGTH_MODIFIERS_CONFIG;

export const modifyContentLength = async (
  text: string,
  ...
  const settings =
    typeof settingsOrModel === "string"
      ? ({ selectedModel: settingsOrModel } as ModelSettings)
      : settingsOrModel;
  const model = settings.selectedModel;

  const modifierConfig = LENGTH_MODIFIERS_CONFIG[modifier];
  const instruction = modifierConfig
    ? modifierConfig.prompt
    : "Modifica el siguiente texto:";
  const prompt = `${instruction}\n\n"${text}"`;
  ...
  return {
    text: response.text || text,
    usage: extractUsage(response, model, "generation", transformSettings, text),
  };
};
...
export const generateMetaFramework = async (
  nicheProblem: string,
  model: GeminiModel,
): Promise<Framework> => {
  if (!ai) throw new Error("API Key no disponible");
...
export const formatText = async (
  ...
  switch (format) {
    case "organize":
      instruction =
        "Organiza y estructura el siguiente texto de manera lógica con encabezados:";
      break;
    case "markdown":
      instruction = "Convierte el siguiente texto a formato Markdown limpio:";
      break;
    case "json":
      instruction =
        "Convierte el contenido del siguiente texto a una estructura JSON válida:";
      break;
  }
  const prompt = `${instruction}\n\n"${text}"`;
  const transformSettings = getTransformationSettings(settings);
  const response = await callGeminiWithRetry({
    ...
    contents: prompt,
    config: buildConfig(transformSettings, true),
  });
  return {
    text: response.text || text,
    usage: extractUsage(
      response,
      transformSettings.selectedModel,
      "generation",
      undefined,
      text,
    ),
...
export const generateRandomIdea = async (settings: ModelSettings) => {
  ...
  const response = await callGeminiWithRetry({
    model: resolveModel(fastSettings.selectedModel),
    contents:
      "Genera una idea creativa, única y específica para un prompt de IA útil. Solo la idea, sin introducción.",
    config: buildConfig(fastSettings, true),
  });
  return {
    text: response.text || "",
    usage: extractUsage(response, fastSettings.selectedModel, "generation"),
  };
};

export const evaluateIdeaQuality = async (
  idea: string,
  settings: ModelSettings,
) => {
...
export const suggestRelatedIdeas = async (
  idea: string,
  files: any[],
  settings: ModelSettings,
) => {
...
export const extractKeyEntities = async (
  idea: string,
  files: any[],
  settings: ModelSettings,
) => {
...
export const performDeepResearch = async (
  ...
  if (frameworks.length > 0 && usage) {
    frameworks[0].usage = usage;
  }
  return frameworks;
};

export const evolvePrompt = async (
...
export const critiqueResponse = async (
  p: string,
  r: string,
  settings: ModelSettings,
) => {
...
export const improvePromptBasedOnAnalysis = async (
  t: string,
  a: any,
  settings: ModelSettings,
) => {
